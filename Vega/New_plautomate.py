...
pl_template = 'symantec_pca.pl.xml'
...
def callModifyPL(top):
	global PL_DIR, repo, repo_relative, PLVersion, buildnum, PLVersion, pl_template, pl_tempdst, plNameVersion, Release
	...
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\lindon_pca_' + plNameVersion + PLVersion + '.pl.xml')
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\LINUSPLXML.txt", "w")
	text_file.write('lindon_pca_' + plNameVersion + PLVersion + '.pl.xml')
	text_file.close()
...
def callModifyRelativ(top):
	...
	cmd_relative = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + pl_template + '" ' + '"' + pl_tempdst + '\\relative_pca_' + plNameVersion + PLVersion + '.pl.xml" ' + (r'"\\\\ali-netapp1.linus.sen.symantec.com\\build\\lindon\\%s" ' % (Release).lower()) + '".."')
	...
	text_file.write('relative_pca_' + plNameVersion + PLVersion + '.pl.xml')
	text_file.close()
...
def callModifyEstoniaUNC(top):
	...
	cmd_estonia = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + pl_template + '" ' + '"' + pl_tempdst + '\\estonia_pca_' + plNameVersion + PLVersion + '.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\eta-netapp1.tales.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_estonia
	os.system(cmd_estonia)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\TALESPLXML.txt", "w")
	text_file.write('estonia_pca_' + plNameVersion + PLVersion + '.pl.xml')
	text_file.close()
...
def callModifyPuneUNC(top):
	...
	cmd_pune = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + pl_template + '" ' + '"' + pl_tempdst + '\\pune_pca_' + plNameVersion + PLVersion + '.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\pun-netapp1.punin.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_pune
	os.system(cmd_pune)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\PUNINPLXML.txt", "w")
	text_file.write('pune_pca_' + plNameVersion + PLVersion + '.pl.xml')
	text_file.close()
...
def copyFinalPL(top):
	...
	global pl_template, pl_tempdst
	VEGAPL_DIR = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\PL' % (Release)
	pl_tempdst = top + r'\CM\PLXML'
	path = top
	text_file=open(path + "\\PLUNC.txt", "w")
	text_file.write(VEGAPL_DIR)
	text_file.close()
	...
	for path, dirs, files in os.walk(pl_tempdst):
		for xml in [os.path.join(path, filename) for filename in files if fnmatch.fnmatch(filename, '*.pl.xml')]:
			print "Copying : ", xml, "to", VEGAPL_DIR
			shutil.copy(xml, VEGAPL_DIR)
	for xml in glob.glob(VEGAPL_DIR + '\\*_Administrator_ITMSCBPLWIN.pl.xml'):
		os.remove(xml)
	if os.path.exists(VEGAPL_DIR + '\\' + pl_template):
		os.remove(VEGAPL_DIR + '\\' + pl_template)
