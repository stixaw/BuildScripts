...
try:
  ...
  SBS_WSI_X86_FILE_NAME = 'Symantec_Deployment_SBS_X86'
  SBS_WSI_X64_FILE_NAME = 'Symantec_Deployment_SBS_X64'
...
#pxe
  SBS_WSI_X86_FILE_NAME_SCRIPT = r'%s\%s.wsi' % (SOLUTION_INSTALL_DIR, SBS_WSI_X86_FILE_NAME)
  SBS_WSI_X64_FILE_NAME_SCRIPT = r'%s\%s.wsi' % (SOLUTION_INSTALL_DIR, SBS_WSI_X64_FILE_NAME)
 ...
 # DRVRDB_INSTALL_64_SCRIPT = r'%s\%s.wsi' % (DRVRDB_INSTALL_DIR, DRVRDB_INSTALL_64_NAME)
  ...
  AGENT_INSTALL_MSI = r'%s\Symantec_DeploymentSolutionAgent_7_1_SP2_x86.msi' % (AGENT_INSTALL_DIR)
  AGENT_64_INSTALL_MSI = r'%s\Symantec_DeploymentSolutionAgent_7_1_SP2_x64.msi' % (AGENT_INSTALL_DIR)
  #AGENT_86_INSTALL_64_MSI = r'%s\Symantec_DeploymentSolutionAgent_7_1_86_x64.msi' % (AGENT_INSTALL_DIR)
  TASKSERVER_INSTALL_MSI = r'%s\Symantec_DeploymentSolutionTaskServerHandler_7_1_SP2_x86.msi' % (TASKSERVER_INSTALL_DIR)
  TASKSERVER_64_INSTALL_MSI = r'%s\Symantec_DeploymentSolutionTaskServerHandler_7_1_SP2_x64.msi' % (TASKSERVER_INSTALL_DIR)
  #MAIN_INSTALL_MSI = r'%s\Symantec_DeploymentSolution_7_1_SP2_x86.msi' % (SOLUTION_INSTALL_DIR)
  MAIN_64_INSTALL_MSI = r'%s\Symantec_DeploymentSolution_7_1_SP2_x64.msi' % (SOLUTION_INSTALL_DIR)
  # SBS_X86_INSTALL_MSI = r'%s\Symantec_Deployment_SBS_7_1_SP2_x86.msi' % (SOLUTION_INSTALL_DIR)
  # SBS_X64_INSTALL_MSI = r'%s\Symantec_Deployment_SBS_7_1_SP2_x64.msi' % (SOLUTION_INSTALL_DIR)
  ...
  #GPL_INSTALL_86_MSI = r'%s\Symantec_NSLINUX_7_1_SP2_x86.msi' % (GPL_INSTALL_DIR)
  GPL_INSTALL_64_MSI = r'%s\Symantec_NSLINUX_7_1_SP2_x64.msi' % (GPL_INSTALL_DIR)
  #DRVRDB_INSTALL_86_MSI = r'%s\Symantec_DriversDatabase_7_1_x86.msi' % (DRVRDB_INSTALL_DIR)
  #DRVRDB_INSTALL_64_MSI = r'%s\Symantecs_DriversDatabase_7_1_x64.msi' % (DRVRDB_INSTALL_DIR)
  #TS_HOTFIX_INSTALL_MSI = r'%s\Symantec_DeploymentSolutionTaskServerHandler_7_1_KB_HF1_x86.msi' % (TASKSERVER_INSTALL_DIR)
  ...
  SIGN_FILES_x86CS = [
  # x86cs binaries
    r'%s\output_win\TaskServerHandler\release\Symantec.Deployment.TaskServerHandler.dll' % top,
    r'%s\output_win\TaskServerHandler\release\policy.7.1.Symantec.Deployment.TaskServerHandler.dll' % top,
  ...
  SIGN_MSI = [
    r'%s\apps\DeploymentServer\install\Symantec_DeploymentSolutionTaskServerHandler_7_1_SP2_x64.msi' % top,
    r'%s\apps\DeploymentServer\install\Symantec_DeploymentSolutionTaskServerHandler_7_1_SP2_x86.msi' % top,
    r'%s\apps\DeploymentInstall\Symantec_DeploymentSolution_7_1_SP2_x64.msi' % top,
    r'%s\apps\DeploymentInstall\Symantec_Deployment_SBS_7_1_SP2_x64.msi' % top,
    r'%s\apps\DeploymentInstall\Symantec_Deployment_SBS_7_1_SP2_x86.msi' % top,
    #r'%s\apps\DeploymentInstall\Altiris_DeploymentSolution_7_1_SP2_x86.msi' % top,
    r'%s\apps\DeploymentClient\install\Symantec_DeploymentSolutionAgent_7_1_SP2_x86.msi' % top,
    r'%s\apps\DeploymentClient\install\Symantec_DeploymentSolutionAgent_7_1_SP2_x64.msi' % top,
    ...
    r'%s\apps\DeploymentInstall\Symantec_NSLINUX_7_1_SP2_x64.msi' % top,
  ...
  Check_List = [
    top + '\\distrib\\imaging\\ghost\\bootwiz\\Bootwiz.exe',
    top + '\\distrib\\imaging\\ghost\\bootwiz\\bootwiz.ini',
    top + '\\distrib\\imaging\\ghost\\bootwiz\\default.bdc',
    top + '\\distrib\\imaging\\ghost\\bootwiz\\BDC_Engine.dll',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x86\\RpcDll.dll',
    top + '\\distrib\\imaging\\ghost\\SBS\\x86\\SbsMtftp.exe',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x86\\SbsNSiFace.dll',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x86\\SbsNSiSignal.exe',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x86\\SbsParsePxe.dll',
    top + '\\distrib\\imaging\\ghost\\SBS\\x86\\SbsServer.exe',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x64\\RpcDll.dll',
    top + '\\distrib\\imaging\\ghost\\SBS\\x64\\SbsMtftp.exe',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x64\\SbsNSiFace.dll',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x64\\SbsNSiSignal.exe',
    #rtop + '\\distrib\\imaging\\ghost\\SBS\\x64\\SbsParsePxe.dll',
    top + '\\distrib\\imaging\\ghost\\SBS\\x64\SbsServer.exe',
  ...
  COPY_TO_PREBOOT_DEST_TASKSFOLDER = r'%s\OEM\DS\winpe\x86\Base\Program Files\Symantec\Deployment' % top
  COPY_TO_PREBOOT_DEST_AGENTFOLDER = r'%s\OEM\DS\winpe\x86\Base\Program Files\Symantec\Deployment' % top
  COPY_TO_PREBOOT_DEST_TASKSFOLDER_X64 = r'%s\OEM\DS\winpe\x64\Base\Program Files\Symantec\Deployment' % top
  COPY_TO_PREBOOT_DEST_AGENTFOLDER_X64 = r'%s\OEM\DS\winpe\x64\Base\Program Files\Symantec\Deployment' % top

  # removed MAIN_INSTALL_MSI and DRVRDB_INSTALL_86_MSI from package x86 till we are told to support NS as a x86 system again
  packages = {
    'x86':(AGENT_INSTALL_MSI, TASKSERVER_INSTALL_MSI),
    'x64':(AGENT_64_INSTALL_MSI, TASKSERVER_64_INSTALL_MSI, MAIN_64_INSTALL_MSI, GPL_INSTALL_64_MSI)
  }

  destinationDirectories = {
    'builddev':r'\\builddev\buildtest\DeploymentSolution\Daily_Builds\ValenciaDev\%MAJOR%.%MINOR%.%BUILD%.0\%PACKAGE%',
...
def buildSolutions():
  global top
  print "Now building Visual Studio Files ..."
  msdev = 'C:\\Program Files'+ ' ' +'(x86)\\Microsoft'+ ' ' + 'Visual' + ' ' + 'Studio' + ' ' + '9.0\\Common7\\IDE\\devenv.exe'
  print msdev
  projectconfig = '"' + 'Release|Any CPU' + '" '
  slnlogfile = top + '\\slnlogfile.txt'
  projectfile = top + '\\apps\\DeploymentServer\\Altiris.Deployment.PSComponent.Services\\Symantec.Ghost.Deployment.PSComponent.Services.csproj'
  Execute(top, '"' + msdev + '" ' + projectfile + ' /rebuild ' + projectconfig + ' /projectconfig ' +  projectconfig + ' /out' + '  ' + slnlogfile)
  
  projectconfig = '"' + 'Release|Win32' + '" '
  slnlogfile = top + '\\slnlogfile.txt'
  projectfile = top + '\\apps\\DeploymentClient\\DSInterOp\\VS2008\\DSInterOp.vcproj'
  Execute(top, '"' + msdev + '" ' + projectfile + ' /rebuild ' + projectconfig + ' /projectconfig ' +  projectconfig + ' /out' + '  ' + slnlogfile)
  projectfile = top + '\\apps\\DeploymentClient\\PECTAgent\\VS2008\\PECTAgent.vcproj'
  #Execute(top, '"' + msdev + '" ' + projectfile + ' /rebuild ' + projectconfig + ' /projectconfig ' +  projectconfig + ' /out' + '  ' + slnlogfile)
...
def buildMSIinWIX():
  ...
  slnlogfile = top + '\\slnlogfile.txt'
  project = 'Ghost.Deployment.Client'
  slnfile = top + '\\apps\\VS2008\DeploymentServer\\DeploymentServer.sln'
  Execute(top, '"' + msdev + '" ' + slnfile + ' /rebuild ' + projectconfig + ' /project ' +  project + ' /out' + '  ' + slnlogfile)
  project = 'Ghost.Deployment'
  Execute(top, '"' + msdev + '" ' + slnfile + ' /rebuild ' + projectconfig + ' /project ' +  project + ' /out' + '  ' + slnlogfile)
    
def buildMSI(msiCommand, scriptPath):

  print 'building msi SourcePath :', scriptPath
...
def buildx86MSIs():
  ...
  buildMSI(msiCommand, SBS_WSI_X86_FILE_NAME_SCRIPT)
...
def buildx64MSIs():
  ...
  print 'Creating x64 MSI\'s...', msiCommand
#pxe
  buildMSI(msiCommand, SBS_WSI_X64_FILE_NAME_SCRIPT)
...
def SignFiles(files):
  ...
  if signFiles:
    privateKey = top +'\\build\\verisign\\Authenticode\\myprivatekey.pvk'
    if os.path.exists(privateKey):
      for fileToSign in files:
        if os.path.exists(fileToSign):
          filename = os.path.basename(fileToSign)
          cmd = 'build\\verisign\\DoPswd.exe signcode -C build\\verisign\\InetSDK\\bin\\SignCode.exe -spc build\\verisign\\Authenticode\\MyCredentials.spc -v ' + privateKey + ' -a SHA1 -t http://timestamp.verisign.com/scripts/timstamp.dll ' + fileToSign + ' -n "' + filename + '"'
          print 'Running "' + cmd + '"...'
          Execute (top, cmd)
        else:
...
def MkBWDirs():
  ...
  if not os.path.exists(top + '\\distrib\\bootwiz\\Platforms\\Winpe\\x86\\Drivers\\custom'):
    os.makedirs(top + '\\distrib\\bootwiz\\Platforms\\Winpe\\x86\\Drivers\\custom')
  if not os.path.exists(top + '\\distrib\\bootwiz\\Platforms\\Winpe\\x64\\Drivers\\custom'):
    os.makedirs(top + '\\distrib\\bootwiz\\Platforms\\Winpe\\x64\\Drivers\\custom')
...
if __name__ == '__main__':
  ...
  try:
    if clean:
      ...
      buildAll()
      buildSolutions()
      SignFiles(SIGN_FILES_x86CS)
      SignFiles(SIGN_FILES_x86CPP)
      SignFiles(SIGN_FILES_x64CPP)
