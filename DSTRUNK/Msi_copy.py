...
def parseArgs(argv):
	global FirstRun, SolName, Src_Dir, Version, top, Sol_DICT
	try:
		opts, args = getopt.getopt(argv, "hf:s:s:b:t:d", [ 'help', 'firstrun', 'solution=', 'srcdir=', 'buildnum=', 'top=', 'dict=' ])
...
def SignFiles(pwd, dir):
	...
	for path, dirs, files in os.walk(dir):
		...
		#for msi in fnmatch.fnmatch(msi,'*.msi'):
			try:
				...
				print "Unable to sign" + msi + " check to see if the file is an msi."
...
def DocCopyRenamSign_MSI(pwd, dict):
	...
	signingToolsDir = top + r'\CM\Verisign'
	sign_tool = top + r'\CM\Verisign\signtool.exe'
	Doc_Path = r'\\ali-netapp1.altiris.com\polaris\ITMS\CombinedBuild\Doc_Builds\Valencia'
...
def get3rdParty_MSI(pwd, sol, dict):
	...
	if sol == 'SEPIC':
		sol_path = r'\\ali-netapp1.altiris.com\polaris\ITMS\CombinedBuild\Daily_Builds\SEPICSolution'
	if sol == 'DMC':
		sol_path = r'\\ali-netapp1.altiris.com\polaris\ITMS\CombinedBuild\Daily_Builds\DMC'
