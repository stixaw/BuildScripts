...
def P4Submit():
  ...
  global workspace, Build, dpt_trunk
...
def Rename_LPMSI():
  global MSI_DIR, top
  print 'MSI_DIR = ', MSI_DIR
  print "renaming x86 MSI"
  if os.path.exists(MSI_DIR + '\\Altiris_DeploymentSolutionLanguages_x86.msi'):
    os.rename(MSI_DIR + '\\Altiris_DeploymentSolutionLanguages_x86.msi', MSI_DIR + '\\DeploymentSolutionLanguages_x86.msi')
  print "renaming x64 MSI"
  if os.path.exists(MSI_DIR + '\\Altiris_DeploymentSolutionLanguages_x64.msi'):
    os.rename(MSI_DIR + '\\Altiris_DeploymentSolutionLanguages_x64.msi', MSI_DIR + '\\DeploymentSolutionLanguages_x64.msi')

def Copy_LPMSI():
  global Build, MSI_DIR, top
  print "build version = ", Build
  BLDLP_DST = r'\\builddev.altiris.com\buildtest\DeploymentSolution\Daily_Builds\trunk\LanguagePacks\%s' % Build
  ...
  # shutil.copy(MSI_DIR + '\\DeploymentSolutionLanguages_x86.msi', 'C:\\output_msi')
    print "copying Altiris_DeploymentSolutionLanguages_7_1_x86.msi to C:\\output_msi"
