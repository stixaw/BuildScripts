...
#Source for version for rdeploy
	if plversion == 'latest':
		...
		f = open ( top + r'\CM\Scripts\Valencia\pl.have', 'r')
		have = f.read()
		have1 = have.replace('_','')
		print 'have =', have
		print 'have1 replace =', have1
		f.close()
		f = open( top + r'\CM\Scripts\Valencia\pl.want', 'r')
		want = f.read()
		print 'want = ', want
		f.close()
...
def UpdateHave(top):
	...
	f = open (top + r'\CM\Scripts\Valencia\pl.have', 'w')
	f.write(str(want))
	f.close()
...
def callModifyPL(top):
	...
	cmd = top + r'\CM\Scripts\Valencia\ITMSCB_SP2.bat ' + PL_DIR + ' ' + Version + ' ' +  repo + ' ' + top + ' ' + buildnum
	print "Calling ", cmd
	os.system(cmd)	
