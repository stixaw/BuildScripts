...
def callModifyPL(top):
	global PL_DIR, repo, repo_relative, PLVersion, buildnum, TIM_STMP, pl_template, pl_tempdst, plNameVersion
	...
	cmd = top + r'\CM\Scripts\CBPITMS_SP2.bat ' + PL_DIR + ' ' + PLVersion + ' ' +  top + ' ' + buildnum
	print "Calling Modifypl with this command: ", cmd
	os.system(cmd)
	#make copy of lindon unc pl:
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\lindon_itms_' + plNameVersion + TIM_STMP + '.pl.xml')
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\LINUSPLXML.txt", "w")
	text_file.write('lindon_itms_' + plNameVersion + TIM_STMP + '.pl.xml')
	text_file.close()

def callModifyRelativ(top):
	global PL_DIR, repo_relative, TIM_STMP, pl_template, pl_tempdst, plNameVersion
	#make copy of relative path pl:
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\relative_itms_' + plNameVersion + TIM_STMP + '.pl.xml') 
	#call modify batch file with linus unc path for NewRepository
	cmd_relative = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\relative_itms_' + plNameVersion + TIM_STMP + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, repo_relative))
	print "Calling Modifypl with this command: ", cmd_relative
	os.system(cmd_relative)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\RelativePLXML.txt", "w")
	text_file.write('relative_itms_' + plNameVersion + TIM_STMP + '.pl.xml')
	text_file.close()

def callModifyEstoniaUNC(top):
	global PL_DIR, repo, repo_relative, estonia_unc, TIM_STMP, pl_template, pl_tempdst, plNameVersion
	#make copy of relative path pl:
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\estonia_itms_' + plNameVersion + TIM_STMP + '.pl.xml') 
	#call modify batch file with linus unc path for NewRepository
	cmd_estonia = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\estonia_itms_' + plNameVersion + TIM_STMP + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, estonia_unc))
	print "Calling Modifypl with this command: ", cmd_estonia
	os.system(cmd_estonia)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\TALESPLXML.txt", "w")
	text_file.write('estonia_itms_' + plNameVersion + TIM_STMP + '.pl.xml')
	text_file.close()

def callModifyPuneUNC(top):
	global PL_DIR, repo, repo_relative, pune_unc, TIM_STMP, pl_template, pl_tempdst, plNameVersion
	#make copy of relative path pl:
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\pune_itms_' + plNameVersion + TIM_STMP + '.pl.xml') 
	#call modify batch file with linus unc path for NewRepository
	cmd_pune = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\pune_itms_' + plNameVersion + TIM_STMP + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, pune_unc))
	print "Calling Modifypl with this command: ", cmd_pune
	os.system(cmd_pune)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\PUNINPLXML.txt", "w")
	text_file.write('pune_itms_' + plNameVersion + TIM_STMP + '.pl.xml')
	text_file.close()
...
def copyFinalPL(top):
	...
	ITMSPL_DIR = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\ITMS\PL' % (Release)
	pl_tempdst = top + r'\CM\PLXML'
	path = top
